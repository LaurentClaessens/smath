...
def BODTooGUhQDy():
    ...
    for tr in trig:
        tr.put_mark(0.2,pspicts=pspicts)
        no_symbol(tr.vertices)

    
    J=Segment(B,A).midpoint()
    med=Segment(C,J)
    trig[0].edges[0].divide_in_two(n=2,d=0.1,l=0.2,angle=45,pspicts=pspicts)
    J.put_mark(0.2,angle=None,added_angle=0,text="\( J\)",pspicts=pspicts)
    ...
    hauteur.divide_in_two(n=2,d=0.1,l=0.2,angle=45,pspicts=pspicts)
    K=hauteur.midpoint()
    K.put_mark(0.2,angle=45,added_angle=0,text="\( K\)",pspicts=pspicts)
    tr2=Polygon(A,K,B)
    #tr2.put_mark(0.2,points_names=" K ",pspicts=pspicts)
