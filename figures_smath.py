...
from phystricksZGZNooJZBEwl import ZGZNooJZBEwl
from phystricksXQKVooWaptGc import XQKVooWaptGc
from phystricksBJOSooIdFKWA import BJOSooIdFKWA
from phystricksPORLooZXtmpv import PORLooZXtmpv
from phystricksQMJQooQratnO import QMJQooQratnO
from phystricksWRBDooZhkhcW import WRBDooZhkhcW
from phystricksLIWGooNsyZBz import LIWGooNsyZBz
from phystricksSMXRooCnrlNw import SMXRooCnrlNw
from phystricksELFXooRwIoJO import ELFXooRwIoJO
from phystricksKEQLooOrxtCN import KEQLooOrxtCN
from phystricksWHWXooTAHTOd import WHWXooTAHTOd
from phystricksKTEDooQRUWXz import KTEDooQRUWXz
from phystricksOAHLooOwLezW import OAHLooOwLezW
from phystricksXKZSooPNxqjW import XKZSooPNxqjW
from phystricksKUZZooNuCtFc import KUZZooNuCtFc
from phystricksVEDYooDMnRha import VEDYooDMnRha
from phystricksGOARooHSKUuw import GOARooHSKUuw
from phystricksFSSAooLvkqAf import FSSAooLvkqAf
from phystricksCBHVooIgPuTm import CBHVooIgPuTm
from phystricksLQHOooWPeagY import LQHOooWPeagY
from phystricksHUGKooTkDsOg import HUGKooTkDsOg
from phystricksVATIooEwQldd import VATIooEwQldd
from phystricksJHVRooIEVJMg import JHVRooIEVJMg
from phystricksLVALooSubNDJ import LVALooSubNDJ
from phystricksPOYHooNCayFj import POYHooNCayFj
from phystricksJYAKooXjBUpt import JYAKooXjBUpt
...
from phystricksBODTooGUhQDy import BODTooGUhQDy
from phystricksDNWGooZENzno import DNWGooZENzno
from phystricksAMXQooFiUqRS import AMXQooFiUqRS
from phystricksAAQGooENsxae import AAQGooENsxae
...
from phystricksYBWRooQjwZuu import YBWRooQjwZuu
...
from phystricksSWYDooBCeFJb import SWYDooBCeFJb
from phystricksYLPFooXzdaFr import YLPFooXzdaFr
from phystricksGJMFooFKVCyK import GJMFooFKVCyK
from phystricksCSUHooJIJzsW import CSUHooJIJzsW
from phystricksUVWMooNNnbbG import UVWMooNNnbbG
from phystricksCXDBooJMfhRr import CXDBooJMfhRr
from phystricksKJWQooEicWkQ import KJWQooEicWkQ
from phystricksSXKLooPzFTnQ import SXKLooPzFTnQ
from phystricksZJATooXEJmyI import ZJATooXEJmyI
from phystricksVUXBooJLuLUk import VUXBooJLuLUk
from phystricksMXXQooRUNiwK import MXXQooRUNiwK
from phystricksMXGVooOFDqLq import MXGVooOFDqLq
from phystricksQCARooMunXfF import QCARooMunXfF
from phystricksSCUKooLMEReM import SCUKooLMEReM
from phystricksPPZRooTpQJOC import PPZRooTpQJOC
from phystricksVNJWooDeKdcy import VNJWooDeKdcy
from phystricksOVATooYCMzBB import OVATooYCMzBB
from phystricksNHGWooBOAkGS import NHGWooBOAkGS
from phystricksPYSIooVNiiuo import PYSIooVNiiuo
from phystricksJSYRooLLggNJ import JSYRooLLggNJ
from phystricksQJGGooLkuRLI import QJGGooLkuRLI
from phystricksIJFGooPLxXAV import IJFGooPLxXAV
from phystricksMOCGooKjSrVV import MOCGooKjSrVV
from phystricksTLEXooPDNwLR import TLEXooPDNwLR
from phystricksFARZooXFSDav import FARZooXFSDav
from phystricksLNQRooVNAJpf import LNQRooVNAJpf
from phystricksMWFBooZHANAt import MWFBooZHANAt
from phystricksHRIVooBXxZES import HRIVooBXxZES
from phystricksPIMUooMLLYDU import PIMUooMLLYDU
from phystricksFSQMooFDOrkR import FSQMooFDOrkR
from phystricksKYVAooVmGYhS import KYVAooVmGYhS
from phystricksMWIPooDUJRIo import MWIPooDUJRIo
from phystricksWIYOooLoAtKD import WIYOooLoAtKD
from phystricksIDKXooPrOOeE import IDKXooPrOOeE
from phystricksWUEUooYWwanB import WUEUooYWwanB
from phystricksNFCHooAFFYPx import NFCHooAFFYPx
from phystricksZUZXooIFCxcB import ZUZXooIFCxcB
from phystricksLOPLooZJmTuk import LOPLooZJmTuk
from phystricksCYLGooSGnRii import CYLGooSGnRii
from phystricksHTGDooAVOWgZ import HTGDooAVOWgZ
from phystricksGRUUooPtPBqp import GRUUooPtPBqp
from phystricksQEPZooNndwiS import QEPZooNndwiS
from phystricksTFDNooJemFMW import TFDNooJemFMW
...
from phystricksCFRQooRohKqN import CFRQooRohKqN
...
from phystricksEIHJooJrGgmN import EIHJooJrGgmN
from phystricksPDNJooMoHgCM import PDNJooMoHgCM
...
def append_picture(fun,number):
    if fun in figures_list_1: 
        print(fun,"is already in the list 1")
    figures_list_1.append(fun)
    if number>=2 :
        if fun in figures_list_2: 
            print(fun,"is already in the list 2")
        figures_list_2.append(fun)
    if number>=3 :
        if fun in figures_list_3: 
            print(fun,"is already in the list 3")
        figures_list_3.append(fun)


figures_list=[]

append_picture(UKRHooEvocBg,1)
append_picture(DNWGooZENzno,1)
append_picture(HMNXfhy,1)
append_picture(CMUTooFyLisx,1)
append_picture(WGhpCVm,1)
append_picture(SCVooUjmESN,1)
append_picture(BPZooBCyuyK,1)
append_picture(GFYooZOvPjP,1)
append_picture(SAZooPjiyOV,1)
append_picture(figurePQKzIRY,1)
append_picture(IllusionNHwEtp,1)
append_picture(CPVooSIVPHp,1)
append_picture(DLPooHXskUP,1)
append_picture(CEZooPzIzzi,1)
append_picture(TJPGooLVSxKK,1)
append_picture(XYWooOPFwaca,1)
append_picture(GVGooXlzMMh,1)
append_picture(SESooBTZQPF,1)
append_picture(NNZooNCAqUK,1)
append_picture(MEWYooRvygNF,1)
append_picture(FQUNooERdWZY,1)
append_picture(MOCGooKjSrVV,1)
append_picture(KRFooUgRQSB,1)
append_picture(ZRDYooCERmeJ,1)
append_picture(XVMLooOHswig,1)
append_picture(LYJooVYRkMy,1)
append_picture(ANRUooVCOTOb,1)
append_picture(SQRooAWmMQL,1)
append_picture(RouletteACaVAA,1)
append_picture(DesSectionseVPNeL,1)
append_picture(TOcdZDG,1)
append_picture(WHWXooTAHTOd,1)
append_picture(SKZKooYWRnFD,1)
append_picture(LXQooZPbZml,1)
append_picture(XFMTooSCJlTh,1)
...
append_picture(KIZooDOXSDH,1)
append_picture(UVWMooNNnbbG,1)
append_picture(EISooBQSffk,1)
append_picture(TBYooGTZFxt,1)


append_picture(HistoTdkccB,2)
append_picture(PDNJooMoHgCM,2)
append_picture(LTenBUj,2)
append_picture(DefVecteurAXDDGP,2)
append_picture(VAAYooXndWQq,2)
append_picture(figureNNgEEzx,2)
append_picture(EABWooPkLqOJ,2)
append_picture(FWyrYhJ,2)
append_picture(MEzTDZC,2)
append_picture(WGCXlvC,2)
append_picture(bDdpfh,2)
append_picture(JFYIooGELsSX,2)
append_picture(YMGAooVCIBvE,2)
append_picture(OBLUooWYQnuB,2)
append_picture(ParabolesoDGyNW,2)
append_picture(FCarreQFhsWz,2)
...
append_picture(IQnEPpt,2)
append_picture(FnInterrobgepC,2)
append_picture(HWLHooGuSDwM,2)
append_picture(FaussePerspectivewAwxAJ,2)
append_picture(GJMFooFKVCyK,2)
append_picture(JOKJMTD,2)
append_picture(XKZSooPNxqjW,2)
append_picture(SVPAooIuxHvP,2)
append_picture(OPZVooLOyZWh,2)
append_picture(AAQGooENsxae,2)
append_picture(RRLOooSCZSre,2)
append_picture(CorSSTXPQVjn,2)
append_picture(XSTKooZqTACG,2)
append_picture(AZKoIsL,2)
append_picture(YLPFooXzdaFr,2)
append_picture(XSMDwcv,2)
append_picture(ZUZXooIFCxcB,2)
append_picture(figureDTzvwiz,2)
append_picture(PythagoreeBqLDU,2)
append_picture(ExoBinomialeNyQLYI,2)
append_picture(FBTCooBKTryQ,2)
append_picture(SurfacesCubesclGZD,2)
append_picture(GOARooHSKUuw,2)
append_picture(VectoParallelgjDlmD,2)
append_picture(QMJQooQratnO,2)
append_picture(JJQWooIxyPzf,2)
append_picture(OKeZlpK,2)
append_picture(YMMDooFDCWPN,2)
append_picture(RAACooAwsaVw,2)
append_picture(DIPLooHILUUs,2)
append_picture(figureERITfSy,2)
append_picture(ILauamX,2)
append_picture(VFAooZmuvtW,2)
append_picture(JHrkjFz,2)
append_picture(YVZAXhU,2)
append_picture(OZXooCVkgQ,2)
append_picture(GSfDCyx,2)
append_picture(YORfWSM,2)
append_picture(OHDIooDdupWC,2)
append_picture(STSooGUprbS,2)
append_picture(ParaboleoytUKk,2)
append_picture(EIHJooJrGgmN,2)
append_picture(JJGooMMFWLs,2)
append_picture(PMVPooKtUXux,2)
append_picture(KSQooHHfEpe,2)
append_picture(CFWIooWDSwRD,2)
append_picture(figureFGgTGJA,2)
append_picture(RHfkPKj,2)
append_picture(VUXBooJLuLUk,2)
append_picture(UKBImJl,2)
append_picture(VGVIooUoZpRA,2)
append_picture(BZRzIsR,2)
append_picture(QCARooMunXfF,2)
append_picture(ETfnbsh,2)
append_picture(VUOCooYkrktO,2)
append_picture(MXGJooGTXbPw,2)
append_picture(RKXHooRkoXOP,2)
append_picture(RKSooSVlhiF,2)
append_picture(VAAooKPNvIs,2)
append_picture(RGRLooTmnwQZ,2)
append_picture(LQHOooWPeagY,2)
append_picture(IFTGooWtjCeQ,2)
append_picture(ExoGraphIneqPgErDr,2)
append_picture(WRBDooZhkhcW,2)
append_picture(RWZooOBMHZ,2)
append_picture(SXKLooPzFTnQ,2)
append_picture(QBDMooGskfKN,2)
append_picture(ITzxISE,2)
append_picture(IXtyOnk,2)
append_picture(LCUooNGZJFk,2)
append_picture(SMXRooCnrlNw,2)
append_picture(NWVMooKshXYo,2)
append_picture(APNeGtp,2)
append_picture(FoncConstFdDkhW,2)
append_picture(ReperexjVyii,2)
append_picture(figureCFoZCYe,2)
append_picture(JLrRNbT,2)
append_picture(ENQhxmG,2)
append_picture(JRCJooPHFcKn,2)
append_picture(figureVNaHvXi,2)
append_picture(XDdastq,2)
append_picture(ParaboleBasfKtFCN,2)
append_picture(OQTDooZHOnob,2)
append_picture(BKWooKbAHbD,2)
append_picture(figureUERGVgS,2)
append_picture(UEqUrvi,2)
append_picture(figureSNpNWPt,2)
append_picture(KQGKooQyKvWm,2)
append_picture(GRUUooPtPBqp,2)
append_picture(YIGPooZbaCAI,2)
append_picture(BAHMooAGUdjV,2)
append_picture(LZKGooYGQxGy,2)
append_picture(figureASkECWS,2)
append_picture(BXFFooNyvPYY,2)
append_picture(FSSAooLvkqAf,2)
append_picture(YDTGooZGkkei,2)
append_picture(YEWSooDXYRJo,2)
append_picture(ChaslesGTRtKR,2)
append_picture(PositionPlansqSltxa,2)
append_picture(WIYOooLoAtKD,2)
append_picture(VUUBooRqdACk,2)
append_picture(KNPRooNdvKmg,2)
append_picture(figureKAzSlQr,2)
append_picture(ZFNXooSjzTPJ,2)
append_picture(ZJATooXEJmyI,2)
append_picture(YBxSjFS,2)
append_picture(QLQYooNbqiXG,2)
append_picture(KTEDooQRUWXz,2)
append_picture(LectureGraphnrkEEM,2)
append_picture(VectoMilieuNuWgHW,2)
append_picture(KEQLooOrxtCN,2)
append_picture(GSPNooCOfCGS,2)
append_picture(FSQMooFDOrkR,2)
append_picture(FLnDVHh,2)
append_picture(ParaboleBasDqAAua,2)
append_picture(ExCarrexvfvre,2)
append_picture(FJPooMGmakN,2)
append_picture(CBHVooIgPuTm,2)
append_picture(NQWWooIaAkTf,2)
append_picture(PerpSegqrbMBZ,2)
append_picture(NARooFiHuAy,2)
append_picture(LignesCubeshBfjxk,2)
append_picture(MinMaxKNRdOd,2)
append_picture(OLuvnaY,2)
append_picture(IKZEooAwkZSy,2)
append_picture(SurfaceCubeXlLEEy,2)
append_picture(IUHooFrAHoa,2)
append_picture(HMUTooSSSCZl,2)
append_picture(TWGooMalxLa,2)
append_picture(ParaboleHautMLbPQF,2)
append_picture(figureXCScSiP,2)
append_picture(UZOQooTSAQcl,2)
append_picture(GrapheVarndvdQM,2)
append_picture(EGDFJAT,2)
append_picture(UPGooXSYGqo,2)
append_picture(DTFZooTlciUT,2)
append_picture(WTVlzUE,2)
append_picture(EDEYRhQ,2)
append_picture(TFUFooJKyBhZ,2)
append_picture(ParaboleUneSolPktmCR,2)
append_picture(HUGKooTkDsOg,2)
append_picture(CSUHooJIJzsW,2)
append_picture(ParabolevQzhjq,2)
append_picture(LXWooEsxsx,2)
append_picture(PORLooZXtmpv,2)
append_picture(QJGGooLkuRLI,2)
append_picture(AMXQooFiUqRS,2)
append_picture(FDOMooPmHEcu,2)
append_picture(KUMFooSHPFkG,2)
append_picture(YRQOoPE,2)
append_picture(KMJQWwa,2)
append_picture(ITlywMb,2)
append_picture(DZmbmzZ,2)
append_picture(HistoAgeFrance,2)
append_picture(UMQMooOlJYbZ,2)
append_picture(TVRGooTuCNyN,2)
append_picture(WUEUooYWwanB,2)
append_picture(ZMGMLvNBa,2)
append_picture(LZDNqtV,2)
append_picture(NHGWooBOAkGS,2)
append_picture(YAFJooWuUmHJ,2)
append_picture(figureEWDVDTS,2)
append_picture(PasFonctionYoQfSu,2)
append_picture(FARZooXFSDav,2)
append_picture(MathCeilwCXIJZ,2)
append_picture(YMslVxg,2)
append_picture(YBWRooQjwZuu,2)
append_picture(BJOSooIdFKWA,2)
append_picture(GrapheVarREGMqx,2)
append_picture(ELFXooRwIoJO,2)
append_picture(PositionsDroitesbnYIsH,2)
append_picture(ExoVectoPJGrRF,2)
append_picture(LRWooCalQsT,2)
append_picture(QGRiHbb,2)
append_picture(RNTRooAXhubs,2)
append_picture(ExoIntersectionCourbenzIxXd,2)
append_picture(OSLooCtzTMX,2)
append_picture(VXDJooBGRpbL,2)
append_picture(figureXNAufCh,2)
append_picture(SYKMooKLBCXR,2)
append_picture(LOBVHYF,2)
append_picture(AWKEooFytNYo,2)
append_picture(PYSIooVNiiuo,2)
append_picture(figureYWEhCkv,2)
append_picture(DSBHooZdaNyy,2)
append_picture(XCWLooVgowie,2)
append_picture(GQVWooMiTJnC,2)
append_picture(DEAXooTRICJi,2)
append_picture(OJDooPOzSiC,2)
append_picture(RYDooDLpToB,2)
append_picture(GJbvyTt,2)
append_picture(KKEdcAR,2)
append_picture(XUBNooNxZEmS,2)
append_picture(JSFooDBSHFo,2)
append_picture(OWGRRSC,2)
append_picture(PYYEasw,2)
append_picture(CubeLFZuiW,2)
append_picture(QLbEnnx,2)
append_picture(OIHVjmO,2)
append_picture(JAQHooIhMeKp,2)
append_picture(OVATooYCMzBB,2)
append_picture(figureCSIQETx,2)
append_picture(MXGVooOFDqLq,2)
append_picture(IRHrmQQ,2)
append_picture(ZQRZooGsxVIr,2)
append_picture(MUriGyU,2)
append_picture(NGWKooMCeSDk,2)
append_picture(figureSIZwqIZ,2)
append_picture(EDnzyzS,2)
append_picture(XPqLRCF,2)
append_picture(ExResolutionOSiaMS,2)
append_picture(CZFJooUDaKCj,2)
append_picture(WRXbDCo,2)
append_picture(CFFyezr,2)
append_picture(GDSEooVtgtKw,2)
append_picture(RZwXiJD,2)
append_picture(GrandeBinomialeIyWAIh,2)
append_picture(IQKJooUmvFBs,2)
append_picture(figureSCkAAJI,2)
append_picture(XUATooMWfESC,2)
append_picture(JLUTBlD,2)
append_picture(NPGPooUwaiww,2)
append_picture(GVPPooOpnIyt,2)
append_picture(EZTHooBttIaQ,2)
append_picture(LOPLooZJmTuk,2)
append_picture(TAIooroZRs,2)
append_picture(KJWQooEicWkQ,2)
append_picture(ZBHLooYfpkZA,2)
append_picture(RVZNtGK,2)
append_picture(GRBOooIZYhIV,2)
append_picture(JQFHooEDhWVO,2)
append_picture(PQYKooJWKpVZ,2)
append_picture(VectoParallItzteT,2)
append_picture(ExoTrigTrIpPW,2)
append_picture(ExIneqOcAWMq,2)
append_picture(JRVlexw,2)
append_picture(CFRQooRohKqN,2)
append_picture(KBOKooTCswpl,2)
append_picture(EvZZys,2)
append_picture(SVPCooGYtlqq,2)
append_picture(CMOooKlzoBL,2)
append_picture(HXDOooHsfhus,2)
append_picture(KSLDooMnmoPU,2)
append_picture(KREWooIrMfCQ,2)
append_picture(JXWXdJI,2)
append_picture(SRYNooEdaMMH,2)
append_picture(figureNPQwFTp,2)
append_picture(AILoxBg,2)
append_picture(ExEquationIntersectioniSHPTw,2)
append_picture(TQDooEgJgPZ,2)
append_picture(OGWJooWdCsvT,2)
append_picture(NUWooJepuh,2)
append_picture(HZPooATdojo,2)
append_picture(ParaboleResumeSzaWaG,2)
append_picture(IERooEvNjp,2)
append_picture(APYEkIv,2)
append_picture(TOIooUindpy,2)
append_picture(WNEooGHwkVN,2)
append_picture(DansRectangleGPEkJc,2)
append_picture(UTDOooJFkZWu,2)
append_picture(BIlgjwy,2)
append_picture(GCxOEgb,2)
append_picture(PJQooTWPTXV,2)
append_picture(IDqyzXM,2)
append_picture(PIMUooMLLYDU,2)
append_picture(IBmsroy,2)
append_picture(PrismeCQlZKY,2)
append_picture(VMNerGf,2)
append_picture(VGQoojvDGr,2)
append_picture(OCFmVDE,2)
append_picture(KXXooKBoqAY,2)
append_picture(PJMMooDNBRCx,2)
append_picture(ZMSWooTAPggA,2)
append_picture(figureSZyxsvp,2)
append_picture(UKTooJhUzKU,2)
append_picture(MITEooHPaqZu,2)
append_picture(ExoVectoFRPXxB,2)
append_picture(IDKXooPrOOeE,2)
append_picture(MWIPooDUJRIo,2)
append_picture(DIDMooOJHGra,2)
append_picture(FDOooRCCWGn,2)
append_picture(CZAVGrm,2)
append_picture(figureISQqBVu,2)
append_picture(KDtwIJf,2)
append_picture(ParaboleHautjOEAzn,2)
append_picture(JSRHooOdlPDT,2)
append_picture(KUZZooNuCtFc,2)
append_picture(ExVariationRXTkoc,2)
append_picture(ITZAPPh,2)
append_picture(NFCHooAFFYPx,2)
append_picture(PEoyeQt,2)
append_picture(GUEjmmR,2)
append_picture(NXOIooIIWUmq,2)
append_picture(AHAbqhj,2)
append_picture(NWPooWQBSnz,2)
append_picture(MWFBooZHANAt,2)
append_picture(CRKLooLhpFTy,2)
append_picture(SCUKooLMEReM,2)
append_picture(MAXkaGz,2)
append_picture(PUGmLBC,2)
append_picture(VDqUhPV,2)
append_picture(figureBCtCTZo,2)
append_picture(DPXooCqCUDl,2)
append_picture(VJZooKxwzhE,2)
append_picture(OTGooGcktOd,2)
append_picture(GraphInterfQVfSf,2)
append_picture(VXUCooAxCIuP,2)
append_picture(WPrirwB,2)
append_picture(ZQfGNsD,2)
append_picture(RMYEooXvicms,2)
append_picture(TCpyyhO,2)
append_picture(GLKFooKwvxSl,2)
append_picture(ParaboleMCLCbG,2)
append_picture(JUZooFbAOlx,2)
append_picture(SWFywZG,2)
append_picture(QDPKooICzieh,2)
append_picture(QTGHooGVusrk,2)
append_picture(ONMRllE,2)
append_picture(OAHLooOwLezW,2)
append_picture(KGOAveW,2)
append_picture(BpCNVm,2)
append_picture(LIWGooNsyZBz,2)
append_picture(HHRTooMDylcn,2)
append_picture(MLZLooYDRsFl,2)
append_picture(MRBWooRCSkaB,2)
append_picture(QFpJtQc,2)
append_picture(ParaboleiLbviP,2)
append_picture(figureBOuQJyj,2)
append_picture(ECQDooWEpuCM,2)
append_picture(RLGQTQR,2)
append_picture(ParaboleUniqueBaskGdqda,2)
append_picture(VNJWooDeKdcy,2)
append_picture(BZqEWco,2)
append_picture(OSQOqJN,2)
append_picture(MTZooSGviQ,2)
append_picture(WYeESAN,2)
append_picture(ZGZNooJZBEwl,2)
append_picture(DDRbyQk,2)
append_picture(figureTFaRFVd,2)
append_picture(PositionRelativexpwkEJ,2)
append_picture(KQluTdN,2)
append_picture(GrapheAffinHqXJGx,2)
append_picture(figureEHyIMRQ,2)
append_picture(HHOooQUedri,2)
append_picture(VJZKooGlvyPP,2)
append_picture(figureFNkqWFE,2)
append_picture(HYKooCotyT,2)
append_picture(ParaboleUniqueHautviflbY,2)
append_picture(figureZEKOYck,2)
append_picture(CylindresxKDOdy,2)
append_picture(POYHooNCayFj,2)
append_picture(LSaSLoS,2)
append_picture(NUEGooGTqxcl,2)
append_picture(KYVAooVmGYhS,2)
append_picture(CXWooOMPOQT,2)
append_picture(PositionPlansTvKvah,2)
append_picture(SUCoowlFdp,2)
append_picture(VXFxyni,2)
append_picture(figureKHUxoaG,2)
append_picture(SurfaceTriangletcNPPE,2)
append_picture(LNQRooVNAJpf,2)
append_picture(figureQUtOjcm,2)
append_picture(ParabolezBeHFl,2)
append_picture(DNYAefI,2)
append_picture(UDKYooKlVbjh,2)
append_picture(AIHBooJaRFHA,2)
append_picture(ZPBMooYYMMgs,2)
append_picture(ExIneqfgZWStde,2)
append_picture(SimulBinNWxfTN,2)
append_picture(VFJVooUXwMvM,2)
append_picture(figureTJkpHLv,2)
append_picture(NAEFooZfyhCs,2)
append_picture(QJLooNAToEq,2)
append_picture(figureLEOvqez,2)
append_picture(XQKVooWaptGc,2)
append_picture(IJFGooPLxXAV,2)
append_picture(ThaleszlOKVq,2)
append_picture(FnAffineipcEQf,2)
append_picture(XTGXooDVcEZs,2)
append_picture(figureMYeLqLl,2)
append_picture(QGaeERu,2)
append_picture(UOEOooLxhpSC,2)
append_picture(YAaXJqQ,2)
append_picture(MJZoobAMRb,2)
append_picture(figureWYxZPMW,2)
append_picture(BJPOooZTVHhi,2)
append_picture(MVHooAdkhKq,2)
append_picture(LVALooSubNDJ,2)
append_picture(LQGzkvL,2)
append_picture(RectanglegHuBZs,2)
append_picture(XMjsBcU,2)
append_picture(SZYooRuSplc,2)
append_picture(OZfyDkr,2)
append_picture(CFjmTFb,2)
append_picture(UZlaYZ,2)
append_picture(CYLGooSGnRii,2)
append_picture(GHKHooEnlVFf,2)
append_picture(EJbcoxO,2)
append_picture(ParabolezmMGdN,2)
append_picture(HistoAutomobile,2)
append_picture(ParaboleResumeHNiyfR,2)
append_picture(MXXQooRUNiwK,2)
append_picture(figureITVTofz,2)
append_picture(figureWFDTzSN,2)
append_picture(EffectifsCumulwfqAhj,2)
append_picture(HistoPySeT,2)
append_picture(PXsdjSu,2)
append_picture(QZABooEsqWaq,2)
append_picture(IsoceleVdviOE,2)
append_picture(GTyQVvw,2)
append_picture(HRIVooBXxZES,2)
append_picture(NIGYQHN,2)
append_picture(CubeLigneTriangleHFMrVU,2)
append_picture(YEGUooNJuuDC,2)
append_picture(figureHYeBZVj,2)
append_picture(HistoAgeKenya,2)
append_picture(ParabolesfTKFw,2)
append_picture(FFyFBoe,2)
append_picture(CVKooPKKMNG,2)
append_picture(figureMIdFCNN,2)
append_picture(JJOooHGTOyn,2)
append_picture(QTCQooFtDgwk,2)
append_picture(JTQooDUZpht,2)
append_picture(KRCFooOBTdqV,2)
append_picture(QEMNooDcNLFy,2)
append_picture(SBRXooGUNvLA,2)
append_picture(SWYDooBCeFJb,2)
append_picture(MEAQooWoXbHZ,2)
append_picture(figureXQZwoWu,2)
append_picture(AVFexUi,2)
append_picture(PetiteBinomialelzbdWl,2)
append_picture(SLULooXaUibV,2)
append_picture(figureNAKnjxQ,2)
append_picture(figureHFdjZpb,2)
append_picture(EIxhcRb,2)
append_picture(HTGDooAVOWgZ,2)
append_picture(TFDNooJemFMW,2)
append_picture(ExoTranslationPNjoHk,2)
append_picture(KYbSnVB,2)
append_picture(PSZooRphUET,2)
...
append_picture(GLVooNqioTo,1)
append_picture(GLVooUSQccD,1)
append_picture(PATooDkWFPD,1)
append_picture(BJDWooWkSKfU,1)
append_picture(QZPMooIiOQpy,1)
append_picture(GARYooJCnpFS,1)
append_picture(RYNYooMFTMNR,1)
append_picture(ENQZooVqRaIv,1)
append_picture(ZUVLooJNWbPB,1)
append_picture(JYAKooXjBUpt,1)
append_picture(JHVRooIEVJMg,1)
append_picture(TLEXooPDNwLR,1)
append_picture(RBEooLYHhMX,1)
append_picture(UFBQooOXIMjV,1)
append_picture(XYXooNdhQer,1)
append_picture(IRQOooJMDFdv,1)
append_picture(LLBQooAjaorQ,1)
append_picture(LWNSooVGvWxJ,1)
append_picture(VATIooEwQldd,1)
append_picture(LVNXooEXEvoV,1)
append_picture(SHSRooHgvofo,1)
append_picture(PPZRooTpQJOC,1)
append_picture(JSYRooLLggNJ,1)
append_picture(RRWJooIPQhTn,1)
append_picture(BODTooGUhQDy,1)
append_picture(KUUooRcCjkQ,1)
append_picture(MBTooHyyNvj,1)
append_picture(YDAooMNHhCN,1)
append_picture(ARKZooKZOuAk,1)
append_picture(TBHUooKbohnF,1)
append_picture(MXHCooEnhHYe,1)
append_picture(PJUIooZuEnPk,1)
append_picture(CSFRooVynjkf,1)
append_picture(FIOJooSmlhXR,1)
append_picture(JMQIooOMgNvh,1)
append_picture(QXYWooOWHshl,1)
append_picture(VEDYooDMnRha,1)
append_picture(FLXBooMvWhiY,1)
append_picture(KPWWooFyZHHV,1)
append_picture(JJHQooEkspEV,1)
append_picture(GCFSooTCJfOZ,1)
append_picture(HYVFooTHaDDQ,1)
append_picture(QAHooOhyyHI,1)
append_picture(QEPZooNndwiS,2)
append_picture(ZSAooVHmHWd,2)
...
def AllFigures():
    ...
    tests=FigureGenerationSuite(figures_list,first=0,title=u"Un soupçon de mathématique")
    tests.generate()
    tests.summary()
