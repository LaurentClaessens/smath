...
\item La \voc{population} désigne l'ensemble des personnes ou objets,
...
\item On recueille alors des données concernant un \voc{caractère} sur
  les individus de cette population. Ce caractère peut prendre un
  certain nombre de \voc{valeurs}, numériques ou non.
  \exe{La taille des élèves de la classe, le salaire des employés, la
  couleur des voitures construites.}
\item Un caractère est dit \voc{quantitatif} lorsqu'il est possible de
  le mesurer en associant un nombre à chaque individu. Dans ce cas, le
  caractère quantitatif est aussi appelé \voc{variable}.
  \exe{L'âge, la taille, le nombre d'enfants dans une famille.}
  \begin{itemize}
  \item Un caractère quantitatif est dit \voc{continu} lorsque les
  ...
  \item Le caractère est dit \voc{discret} dans le cas contraire.
    \exe{L'année de naissance, le nombre de frères et s¿urs.}
  \end{itemize}
\item On appelle caractère \voc{qualitatif} tout caractère qui n'est
  pas quantitatif.
...
On note généralement $N$ l'\voc{effectif total},
...
Le \voc{mode} est la valeur ayant le plus grand effectif. Elle a un
...
A partir du tableau 1, on peut dresser le tableau des \voc{effectifs
cumulés croissants}.
\medskip
...
\rmq{Le tableau des effectifs cumulés donne directement le nombre de
...
\rmq{Le tableau des effectifs cumulés permet également de répérer
...
Par définition, la \voc{fréquence} d'une valeur du caractère $x_i$ est :
\[
\mbox{fréquence} 
...
\voc{fréquences cumulées croissantes}, en ajoutant à chaque fréquence
...
L'\voc{amplitude} de la classe est $b-a$, c'est l'écart entre la plus
grande et la plus petite valeur de la classe.

Le \voc{centre} de la classe est la moyenne $\dfrac{a+b}2$.

La \voc{classe modale} est la classe qui a le plus grand effectif. 
...
\definition{
  La \voc{moyenne} de la série statistique des $(x_i;n_i)$ est le
...
\rmq{On peut aussi calculer $\overline{x}$ en utilisant les fréquences
  $f_i$ : \\[1ex]
...
\definition{  
  La \voc{médiane}, notée $M_e$, d'une série statistique est un
...
\parag{En pratique :}{On range les $N$ valeurs de la série par ordre
  croissant, chacune étant répétée autant de fois que son effectif.
}

\begin{enumerate}[\quad $\longrightarrow$]
...
\paragit{Application :}{Dans chaque cas, déterminer la médiane de la
  série.\\
...
\item 5 ; 10 ; 17 ; 12 ; 6 \\[0.8cm]
\item 1\,000 ; 1\,000 ; 1\,200 ; 1\,200 ; 1\,200 ; 1\,500 ; 1\,500 ;
  2\,000 ; 2\,500 ; 3\,100 \\[0.8cm]
\item 5 ; 1 ; 2 ; 9 ; 4 ; 5 ; 7 ; 5 ; 9 ; 5 ; 6 ; 7 ; 8 ; 9 ; 2 ; 9 ;
  6 ; 5 ; 9 ; 3 \\[1.5cm]
\end{enumerate}

\rmq{La médiane ne dépend pas des valeurs extrêmes.}
...
\definition{
  On considère une liste de $N$ valeurs, \underline{triées par ordre
    croissant}. 
  \begin{enumerate}[\textbullet]
  \item Le \voc{premier quartile} $Q_1$ est la plus petite valeur de
    la liste telle qu'au moins un quart des valeurs de la liste sont
    inférieures ou égales à $Q_1$.
  \item Le \voc{troisième quartile} $Q_3$ est la plus petite valeur de
...
\paragit{Application :}{Déterminer le premier et le troisième quartile
  de chaque série.}

\begin{enumerate}[\quad (a)]
\item 1\,000 ; 1\,000 ; 1\,200 ; 1\,200 ; 1\,200 ; 1\,500 ; 1\,500 ;
  2\,000 ; 2\,500 ; 3\,100 \\[1cm]
\item  5 ; 1 ; 2 ; 9 ; 4 ; 5 ; 7 ; 5 ; 9 ; 5 ; 6 ; 7 ; 8 ; 9 ; 2 ; 9 ;
  6 ; 5 ; 9 ; 3\\[1.5cm]
\end{enumerate}
...
\begin{enumerate}[\textbullet]
\item L'\voc{étendue} d'une série statistique est la différence entre
  la plus grande et la plus petite valeur.
\item L'\voc{écart interquartile} est égal à la différence $Q_3-Q_1$.
\end{enumerate}
...
\paragit{\phantom{Exemple :}}{Le premier quartile vaut \ $Q_1=\comp$. \\[1ex]
  Le troisième quartile vaut \ $Q_3=\comp$. }
