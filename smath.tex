% This is part of Un soupçon de mathématique sans être agressif pour autant
% Copyright (c) 2012-2013
%   Laurent Claessens
% See the file fdl-1.3.txt for copying conditions.

\documentclass[a4paper,12pt]{book}
\input{e_smath}

%\makeindex
%\makenomenclature

%TODO : il faudra fusioner avec echa un jour; oui monsieur : tout echa même la physique.

\begin{document}

\corrPosition{2}
\corrDraft{1}

\input{gardeSmath}

\newpage

\input{introSmath}

\tableofcontents

\newpage

%&=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& 
\part{Seconde}
%&=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& 

\setcounter{chapter}{-1}

\chapter{Rappels de géométrie}
\input{rappels_geom}

\chapter{Statistique descriptive}
\input{stat_descriptives}

\chapter{Repères, distances, milieux}
\input{repere}

\chapter{Généralités sur les fonctions}
\input{fonctions}

\chapter{Géométrie dans l'espace et perspective cavalière}
\input{perspective}

\chapter{Vecteurs}
\input{vecteurs}

\chapter{Fonctions affines}
\input{affines}

\chapter{Probabilités}
\input{proba}

\chapter{La fonction carré}
\input{fonction_carre}

\chapter{La fonction inverse}
\input{fonction_inverse}

\chapter{Intervalle de fluctuation et de intervalle confiance}
\input{IFetIC}

\chapter{Inéquations : tableaux de signes de produits}
\input{ineqs}

\chapter{Les fonctions du second degré}
\input{second_deg}

\chapter{Configurations du plan}
\input{Config_plan}

\chapter{Intersections de droites et systèmes d'équations}
\input{systeme}

\chapter{Fonctions homographiques}
\input{fonction_homographique}

\chapter{Trigonométrie}
\input{trigono}

\chapter{Techniques de calcul}
\input{calcul_mental}

\chapter{Algorithmique}
\input{algo}

\chapter{Les épaves}
\input{les_epaves}

%&=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& 
\part{Première STMG}
%&=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& 
\chapter{Proportions}
\input{proportions}

\chapter{Second degré, paraboles}
\input{parabole}

\chapter{Binomiale}
\input{Binomiale}

\chapter{Évolutions}
\input{evolution}

\chapter{Statistiques}
\input{statistique_stmg}

\chapter{Suites numériques}
\input{suites_numer}

\chapter{Nombre dérivé et tangente}
\input{derr_tangente}

\chapter{Suites arithmétiques}
\input{suites_arith}

\chapter{Fonction dérivée et variations}
\input{derr_var}

\chapter{Intervalles de confiance}
\input{intervalle_confiance}

\chapter{Suites géométriques}
\input{suites_geom}

\chapter{Troisième degré}
\input{troisieme_deg}

%&=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& 
\part{Python}
%&=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& &=& 
\input{APpython}

\part{Autres}
\input{autres}
%\input{de_svt}

\corrChapitre{Corrigés systématiques}

\input{fdl-1.3.tex}

\bibliographystyle{unsrt}           % unsrt fait que la biblio arrive dans l'ordre de citation au lieu de l'ordre alphabétique.
\bibliography{mazhe}

\addcontentsline{toc}{chapter}{Liste des notations}

\printnomenclature

\printindex

\end{document}

\Exo{smath-0334}
\Exo{smath-0335}
\Exo{smath-0336}
\Exo{smath-0337}
\Exo{smath-0338}
\Exo{smath-0339}
\Exo{smath-0340}
\Exo{smath-0341}
\Exo{smath-0342}
\Exo{smath-0343}
\Exo{smath-0344}
\Exo{smath-0345}
\Exo{smath-0346}
\Exo{smath-0347}
\Exo{smath-0348}
\Exo{smath-0349}
\Exo{smath-0350}
\Exo{smath-0351}
\Exo{smath-0352}
\Exo{smath-0353}
\Exo{smath-0354}
\Exo{smath-0355}
\Exo{smath-0356}
\Exo{smath-0357}
\Exo{smath-0358}
\Exo{smath-0359}
\Exo{smath-0360}
\Exo{smath-0361}
\Exo{smath-0362}
\Exo{smath-0363}
\Exo{smath-0364}
\Exo{smath-0365}
\Exo{smath-0366}
\Exo{smath-0367}
\Exo{smath-0368}
\Exo{smath-0369}
\Exo{smath-0370}
\Exo{smath-0371}
\Exo{smath-0372}
\Exo{smath-0373}
\Exo{smath-0374}
\Exo{smath-0375}
\Exo{smath-0376}
\Exo{smath-0377}
\Exo{smath-0378}
\Exo{smath-0379}
\Exo{smath-0380}
\Exo{smath-0381}
\Exo{smath-0382}
\Exo{smath-0383}
\Exo{smath-0384}
\Exo{smath-0385}
\Exo{smath-0386}
\Exo{smath-0387}
\Exo{smath-0388}
\Exo{smath-0389}
\Exo{smath-0390}
\Exo{smath-0391}
\Exo{smath-0392}
\Exo{smath-0393}
\Exo{smath-0394}
\Exo{smath-0395}
\Exo{smath-0396}
\Exo{smath-0397}
\Exo{smath-0398}
\Exo{smath-0399}
\Exo{smath-0400}

\lstinputlisting{ex_algo18.py}
\lstinputlisting{ex_algo19.py}
\lstinputlisting{ex_algo20.py}
\lstinputlisting{ex_algo21.py}
\lstinputlisting{ex_algo22.py}
\lstinputlisting{ex_algo23.py}
\lstinputlisting{ex_algo24.py}
\lstinputlisting{ex_algo25.py}
\lstinputlisting{ex_algo26.py}
\lstinputlisting{ex_algo27.py}
\lstinputlisting{ex_algo28.py}
\lstinputlisting{ex_algo29.py}
\lstinputlisting{ex_algo30.py}
\lstinputlisting{ex_algo31.py}
\lstinputlisting{ex_algo32.py}
\lstinputlisting{ex_algo33.py}
\lstinputlisting{ex_algo34.py}
\lstinputlisting{ex_algo35.py}
\lstinputlisting{ex_algo36.py}
\lstinputlisting{ex_algo37.py}
\lstinputlisting{ex_algo38.py}
\lstinputlisting{ex_algo39.py}
\lstinputlisting{ex_algo40.py}
\lstinputlisting{ex_algo41.py}
\lstinputlisting{ex_algo42.py}
\lstinputlisting{ex_algo43.py}
\lstinputlisting{ex_algo44.py}
\lstinputlisting{ex_algo45.py}
\lstinputlisting{ex_algo46.py}
\lstinputlisting{ex_algo47.py}
\lstinputlisting{ex_algo48.py}
\lstinputlisting{ex_algo49.py}
\lstinputlisting{ex_algo50.py}
\lstinputlisting{ex_algo51.py}
\lstinputlisting{ex_algo52.py}
\lstinputlisting{ex_algo53.py}
\lstinputlisting{ex_algo54.py}
\lstinputlisting{ex_algo55.py}
\lstinputlisting{ex_algo56.py}
\lstinputlisting{ex_algo57.py}
\lstinputlisting{ex_algo58.py}
\lstinputlisting{ex_algo59.py}
\lstinputlisting{ex_algo60.py}
\lstinputlisting{ex_algo61.py}
\lstinputlisting{ex_algo62.py}
\lstinputlisting{ex_algo63.py}
\lstinputlisting{ex_algo64.py}
\lstinputlisting{ex_algo65.py}
\lstinputlisting{ex_algo66.py}
\lstinputlisting{ex_algo67.py}
\lstinputlisting{ex_algo68.py}
\lstinputlisting{ex_algo69.py}
\lstinputlisting{ex_algo70.py}
\lstinputlisting{ex_algo71.py}
\lstinputlisting{ex_algo72.py}
\lstinputlisting{ex_algo73.py}
\lstinputlisting{ex_algo74.py}
\lstinputlisting{ex_algo75.py}
\lstinputlisting{ex_algo76.py}
\lstinputlisting{ex_algo77.py}
\lstinputlisting{ex_algo78.py}
\lstinputlisting{ex_algo79.py}
\lstinputlisting{ex_algo80.py}
\lstinputlisting{ex_algo81.py}
\lstinputlisting{ex_algo82.py}
\lstinputlisting{ex_algo83.py}
\lstinputlisting{ex_algo84.py}
\lstinputlisting{ex_algo85.py}
\lstinputlisting{ex_algo86.py}
\lstinputlisting{ex_algo87.py}
\lstinputlisting{ex_algo88.py}
\lstinputlisting{ex_algo89.py}
\lstinputlisting{ex_algo90.py}
\lstinputlisting{ex_algo91.py}
\lstinputlisting{ex_algo92.py}
\lstinputlisting{ex_algo93.py}
\lstinputlisting{ex_algo94.py}
\lstinputlisting{ex_algo95.py}
\lstinputlisting{ex_algo96.py}
\lstinputlisting{ex_algo97.py}
\lstinputlisting{ex_algo98.py}


PROGRESSION STMG
----------------

Proportions, pourcentages 7 sept -> 17 sept (1.5)
Second degré 18 sept -> 16 oct (4s)
Loi binomiale et binomiale 16oct -> 26 nov (5 semaines)
Taux d'évolution, évolutions successives    (3 semaines, fini juste avant les vacances de noël)

NOËL

Comparaison de séries statistiques; on introduit l'écart-type et les diagrammes en boîte. (3 semaines)

Tableur : tableau d'effectifs et de fréquences  (2 semaine)

Suites numériques (1 semaines)
Nombre dérivé et tangente (2 semaines) Reparler des équations de droites. Surtout retrouver l'équation de la droite avec le coefficient directeur et un point.
Suites arithmétiques (1s)
Fonction dérivée et variations (1)
Intervalles de fluctuation et prise de décision (3 semaines)    À ce niveau il faut faire les derniers trucs sur la binomiale qui ne sont
                                                                pas sur les feuilles déjà distribuées.
Suites géométriques (2 s)
Troisième degré (1.5 semaines)

En info pour :
* statistiques
* évolutions
* suites

Notes pour la prochaine fois 
* il vaut mieux mettre les proportions et sous-populations avant la binomiale parce que ça introduit les arbres de façon plus naturelle.
* pour le second degré, il faut diviser en deux parties : une qui ne fait que ce qui a été vu en seconde et voir les racines et la factorisation plus tard, et probablement en fait encore en deux parties.
* Le chapitre «évolution» peut encore être divisé en «évolution», «tva» et «évolutions successives».



PROGRESSION 2nd
---------------

À faire avec les 2nd2 :
- le chapitre sur la perspective.
À faire avec les 2nd2 :
- intersections de droites
                    
Volumes (3s) : une sur les petites surfaces dans les cubes et 2 les positions de droites et plans dans l'espace
                     %TODO : les calculs de volumes et d'aires doivent être séparés du ``cœur'' du chapitre.

Vecteurs (3s) : à diviser en deux : une fois sans coordonnées, et une fois avec.
                Note pour la prochaine fois : essayer de voir Chasles avant de voir la définition formelle d'une translation.

Fonctions de référence ax+b : équations de droites (3s) -- il faut faire plus de drill sur les équations passant par deux points.

Fonction de référence : x^2 (2s)
Probabilités (1s) 
Fonction de référence : 1/x (2s)

Échantillonnage, intervalle de confiance et de fluctuation (1s)

Fonctions homographiques.

Expressions algébriques (dissimulées un peu partout)
second degré : graphe et tableau de variations (2s)
Inéquations : tableau de signe de produits (1s)

Configurations du plan (2s)         # Les exos de cette partie sont en fait ceux de géométrie «pure» faits plus au début.
                                    % TODO: Il faudrait les séparer pour en faire un chapitre à part entière.



Trigonométrie (1s)

En info pour
* de l'algo
* les stats

Notes pour la prochaine fois.
* Le chapitre équations affines doit être divisé en
        - équations de droites (passant par des points donnés et de vecteur directeur donné)
        - tableau de variation et inéquations.
        - intersections et systèmes.
