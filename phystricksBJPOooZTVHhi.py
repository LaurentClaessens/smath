...
def BJPOooZTVHhi():
    ...
    m1=Segment(R,A).get_measure(0.3,-0.1,None,"\( 2\)",pspict=pspict,position="corner")
    m2=Segment(A,M).get_measure(0.3,-0.1,None,"\( 6\)",pspict=pspict,position="corner")

    trig=Polygon(R,Z,M)
    seg=Segment(I,A)
    trig.put_mark(0.2,text_list=["\( R\)","\( Z\)","\( M\)"],pspict=pspict)

    I.put_mark(0.2,None,"\( I\)",pspict=pspict,position="corner")
