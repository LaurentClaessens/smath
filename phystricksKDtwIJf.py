...
def KDtwIJf():
    ...
    m1.put_mark(0.3,m1.advised_mark_angle,"Environ \( \\frac{ 1 }{ 4 }\)")

    sm2=Segment(Point(minimum,1.5),Point(Q3,1.5))
    m2=MeasureLength(sm2,0.2)
    m2.put_mark(-0.3,m2.advised_mark_angle,"Environ \( \\frac{ 3 }{ 4 }\)")

    sm3=Segment(Point(Q3,0),Point(Maximum,0))
    m3=MeasureLength(sm3,0.2)
    m3.put_mark(0.3,m3.advised_mark_angle,"Environ \( \\frac{ 1 }{ 4 }\)")
